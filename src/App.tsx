...
function App() {
  const [activeSection, setActiveSection] = useState<'projects' | 'stack' | 'papers' | 'blog' | null>(null);

  const scrollToSection = (section: 'projects' | 'blog' | 'stack' | 'papers' ) => {
              ...
              <div>
                ...
                <div className="min-h-screen flex flex-col items-center justify-center px-4 text-center relative z-10">
                  ...
                  <div className="flex flex-col gap-4 mb-12">
                    {['projects', 'blog', 'papers', 'stack'].map((section) => (
                      <button
                        key={section}
                        onClick={(e) => {
                          e.preventDefault();
                          scrollToSection(section as 'projects' | 'stack' | 'papers' | 'blog');
                        }}
                        className={`px-8 py-3 border-2 rounded-lg text-lg font-semibold transition-colors w-48 bg-transparent ${
                          section === 'projects' ? 'border-blue-600 text-blue-600 hover:border-blue-700 hover:text-blue-700' :
                          section === 'papers' ? 'border-green-600 text-green-600 hover:border-green-700 hover:text-green-700' :
                          section === 'stack' ? 'border-purple-600 text-purple-600 hover:border-purple-700 hover:text-purple-700' :
                          'border-yellow-600 text-yellow-600 hover:border-yellow-700 hover:text-yellow-700'
                        }`}
                      >
